...
import { CommentDeleteTable } from "@/components/admin/CommentDeleteTable";
...
export default function FeedbackPage() {
	const [filters, setFilters] = useState<{
		...
		eventId: string;
		search: string;
		searchBy: "user" | "event" | "comment" | "all";
	}>({
		page: 1,
		limit: 10,
		sortBy: "createdAt",
	...
	return (
		<div className="w-full mx-auto space-y-4">
			...
			<section className="space-y-6">
				<div className="flex md:flex-row flex-col gap-4">
					<div className="flex-1">
						<Input
							...
							placeholder="Search by user name, event title, comment, etc."
						...
						<Select onValueChange={handleEventChange} value={filters.eventId || "all"}>
							...
							<SelectContent className="max-h-[50vh] overflow-y-auto">
								...
								{publishedEvents?.data.map((event) => (
									<SelectItem key={event.id} className="capitalize" value={event.id}>
										{event.title}
									</SelectItem>
								))}
							</SelectContent>
				...
				<div className="space-x-2 flex justify-end gap-2">
					<Badge
						variant="outline"
